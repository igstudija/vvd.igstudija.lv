<table class="table table-hover mb-3" id="<?php echo $table_id; ?>-table">
    <thead>
        <tr>
    ...
    <?php if($table_type == 'form'){      
         $table_data = file_get_contents('table-data/' . $table_id . '-data.json');
    ...
    <tr>
         ...
         <?php foreach($table_cols as $table_col){ ?>
            <td>
                ...
                <?php if(array_key_exists('placeholder', $table_col['field_elements'])) {$placeholder = $table_col['field_elements']['placeholder']; }else {$placeholder = NULL;}?>
                <?php if(array_key_exists('id', $rows)) {$id = $row['id']; }else {$id = NULL;}?>
                <?php if(array_key_exists('options', $table_col)) {$options = array($row['id']=>$table_col['options']); }else {$options = NULL;}?>
...
<script>
    $(document).ready(function () {
        $('#<?php echo $table_id; ?>-table').DataTable({
            ...
            <?php if($table_type != 'form'){ ?>
            ,
            "ajax": "table-data/<?php echo $table_id; ?>-data.json"
            <?php } ?>
